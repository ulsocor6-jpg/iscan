...
type Account = {
  ...
  // NOTE: /api/v1/treasury/accounts does NOT return a pre-computed
  // `available` field (confirmed by how PoolCard in Treasury.tsx computes
  // it locally too) — so we compute it ourselves via availableOf() below
  // rather than trusting a field that isn't actually present on this
  // endpoint's response shape.
};

function availableOf(a: Account): number {
  return a.physicalBalance - a.reserved - a.safetyReserve;
}
            ...
            <select style={selectStyle} value={rbSource} onChange={e => setRbSource(e.target.value)}>
              ...
              {phpAccounts.map(a => (
                <option key={a._id} value={a._id}>
                  {a.label} ({a.provider}) — ₱{availableOf(a).toFixed(2)} available
                </option>
              ))}
            </select>
          </div>
