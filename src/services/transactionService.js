...
import { deriveBaseAddress, deriveRoninAddress } from './hdWalletService.js';
...
const P2P_GAS_UNITS = 100_000n;
const P2P_GAS_BUFFER_PCT = 130n;

async function ensureSenderHasGas({ chain, provider, config, senderAddress }) {
  const nativeBalance = await provider.getBalance(senderAddress);
  const feeData = await provider.getFeeData();
  const gasPrice = feeData.gasPrice ?? feeData.maxFeePerGas;
  if (!gasPrice) return;
  const requiredWei = (gasPrice * P2P_GAS_UNITS * P2P_GAS_BUFFER_PCT) / 100n;
  if (nativeBalance >= requiredWei) return;

  const privateKey = config.treasuryPrivateKey();
  if (!privateKey) throw new Error('No treasury private key configured for ' + chain + ' - cannot fund sender gas');

  const treasurySigner = new ethers.Wallet(privateKey, provider);
  const topUpWei = requiredWei - nativeBalance;
  console.log('[P2P] Funding sender gas: ' + ethers.formatEther(topUpWei) + ' native to ' + senderAddress + ' on ' + chain);
  const fundTx = await treasurySigner.sendTransaction({ to: senderAddress, value: topUpWei });
  await fundTx.wait();
}

async function transferStablecoinOnChain({ senderWallet, receiverWallet, amount, asset, chain }) {
  const config = CHAIN_NATIVE_CONFIG[chain?.toLowerCase()];
  if (!config) throw new Error('Unsupported chain for on-chain transfer: ' + chain);

  const senderEntry = senderWallet.chainAddresses?.find(c => c.chain?.toLowerCase() === chain.toLowerCase());
  const receiverEntry = receiverWallet.chainAddresses?.find(c => c.chain?.toLowerCase() === chain.toLowerCase());
  if (!senderEntry?.address) throw new Error('Sender has no ' + chain + ' address');
  if (!receiverEntry?.address) throw new Error('Receiver has no ' + chain + ' address');

  const onChainBalance = await getTokenBalance(chain.toUpperCase(), senderEntry.address, asset);
  if (onChainBalance === null) throw new Error(asset + ' not supported on ' + chain);
  if (onChainBalance < amount) {
    throw new Error('On-chain balance mismatch: sender has ' + onChainBalance + ' ' + asset + ' on-chain, claims ' + amount);
  }

  if (senderWallet.walletIndex === undefined || senderWallet.walletIndex === null) {
    throw new Error('Sender has no walletIndex - cannot sign on-chain transfer');
  }

  const derived = await config.deriveAddress(senderWallet.walletIndex);
  if (!derived?.privateKey) throw new Error('Could not derive sender private key');
  if (derived.address.toLowerCase() !== senderEntry.address.toLowerCase()) {
    throw new Error("Sender's stored address does not match its derived address - refusing to sign");
  }

  const gasCostUSD = await estimateGasCostUSD(chain).catch(() => 0);
  const amountToReceiver = +(amount - gasCostUSD).toFixed(6);
  if (amountToReceiver <= 0) {
    throw new Error('Transfer amount too small to cover network gas cost (~' + gasCostUSD.toFixed(4) + ' ' + asset + ')');
  }

  const provider = new ethers.JsonRpcProvider(config.rpcUrl());
  await ensureSenderHasGas({ chain, provider, config, senderAddress: senderEntry.address });

  const signer = new ethers.Wallet(derived.privateKey, provider);
  const tokenAddress = config.tokens()[asset];
  if (!tokenAddress) throw new Error('No ' + asset + ' contract configured on ' + chain);
  const contract = new ethers.Contract(tokenAddress, ERC20_ABI, signer);
  const decimals = await contract.decimals();
  const amountWei = ethers.parseUnits(amountToReceiver.toString(), decimals);

  console.log('[P2P] Sending ' + amountToReceiver + ' ' + asset + ' on ' + chain + ': ' + senderEntry.address + ' -> ' + receiverEntry.address);
  const tx = await contract.transfer(receiverEntry.address, amountWei);
  const receipt = await tx.wait();
  console.log('[P2P] Confirmed - tx: ' + receipt.hash);
