#!/usr/bin/env bash
# ISCAN — Frontend OTP step patch (Login.tsx + authService.ts)
set -e

if [ ! -f "src/pages/Login.tsx" ]; then
  echo "ERROR: run this from the iscansystem repo root (src/pages/Login.tsx not found here)."
  exit 1
fi

STAMP=$(date +%Y%m%d-%H%M%S)
BACKUP_DIR=".patch-backup-frontend-$STAMP"
mkdir -p "$BACKUP_DIR"

echo "==> Backing up files to $BACKUP_DIR"
cp src/pages/Login.tsx "$BACKUP_DIR/Login.tsx.bak"
cp src/services/authService.ts "$BACKUP_DIR/authService.ts.bak"

echo "==> Writing new src/services/authService.ts"
cat > src/services/authService.ts << 'AUTHSERVICE_EOF'
...
echo "==> Writing new src/pages/Login.tsx"
...
echo "Restart vite (or let it hot-reload) and try logging in again."
